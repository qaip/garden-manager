...
class Handler(GardenHandler):
    def __init__(self, server=False):
        ...
        if bool(self.args['bed_id']) == bool(self.args['all']):
            self.print(
                "Please specify either '--bed BED_ID' argument or '--all' flag")
            exit(1)
        if self.args['bed_id']:
            ...
            if garden_bed is None:
                self.print(f"Failed to harvest from bed: "
    ...
    def harvest(self, query: Query):
        ...
        if result:
            if self.args['all']:
                self.print(f"Harvested {result} from the the whole garden")
            else:
                self.print(
                    f"Harvested {result} from garden bed {self.args['bed_id']}")
        else:
            if self.args['all']:
                self.print("Nothing to harvest from the garden")
            else:
                self.print(
